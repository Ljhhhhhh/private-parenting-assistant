...
- **背景色**：使用从浅蓝到浅粉的渐变 `from-[#E8F0FE] to-[#FFF0F5]`
- **卡片样式**：
  - 圆角：24px
...
- **输入框**：
  ...
  - 边框色：浅蓝 `border-[#E8F0FE]`
  - 焦点状态：蓝色边框 `focus:border-[#4A90E2]`，略微增加透明度 `focus:bg-white/90`
  - 左侧图标：主色 `text-[#4A90E2]`，固定位置 `absolute left-4 top-1/2 -translate-y-1/2`
  - 内边距：左侧有图标时 `pl-12 pr-4`，无图标时 `px-4`
  - 过渡效果：`transition-all duration-300`
- **按钮**：
  - 高度：56px
  - 圆角：16px
  - 颜色：主蓝渐变 `from-[#4A90E2] to-[#7AADEE]`
  - 文字：白色，居中，字体大小 16px `text-base`
  - 阴影：`shadow-lg shadow-[#4A90E2]/20`
  - 悬停状态：更深阴影 `hover:shadow-xl hover:shadow-[#4A90E2]/30`
  - 过渡效果：`transition-all duration-300`
  - 右侧箭头：表示前进/确认的视觉暗示
- **复选框**：
  - 尺寸：20px x 20px
  ...
  - 选中：主色背景，白色勾选图标
  - 标签文字：灰色，左侧间距 8px
- **链接文本**：主色 `text-[#4A90E2]`，中等字重 `font-medium`
...
- **背景色**：与登录/注册页保持一致，使用从浅蓝到浅粉的渐变 `from-[#E8F0FE] to-[#FFF0F5]`
- **卡片样式**：
  - 延续认证页面风格，使用圆角 24px
  - 背景：半透明白色 `bg-white/80` 配合 `backdrop-blur-md` 实现毛玻璃效果
  - 边框：白色半透明 `border border-white/50`
  - 阴影：轻微 `shadow-lg`
- **装饰元素**：
  - 卡片顶部可添加装饰性模糊渐变圆形 `bg-gradient-to-r from-[#4A90E2]/20 to-[#F8BBD0]/30 blur-xl`
...
- **性别选择器**：
  - 两个并排选项，等宽布局 `flex-1`
  - 圆角：12px
  - 高度：52px-56px（适合手指轻松点击）
  - 未选中：浅灰边框 `border-[#E8F0FE]`，半透明白色背景 `bg-white/70`
  - 男孩选中状态：浅蓝背景 `bg-[#E8F0FE]`，蓝色边框和文字 `text-[#4A90E2]`，蓝色图标
  - 女孩选中状态：浅粉背景 `bg-[#FFDCE8]`，粉色边框和文字 `text-[#E091B1]`，粉色图标
  - 过渡效果：`transition-all duration-200`
- **过敏源标签**：
  - 圆角：完全圆角 `rounded-full`
  - 内边距：水平 16px，垂直 8px `px-4 py-2`
  - 未选中：浅灰边框，半透明白色背景，灰色文字
  - 选中：浅蓝背景 `bg-[#4A90E2]/10`，蓝色边框和文字，左侧小勾选图标
  - 过渡效果：`transition-all duration-200`
- **提交按钮**：与登录/注册按钮保持一致
  - 高度：56px
  - 蓝色渐变背景
